...
|Prime    | Prime is _related_ or _unrelated_ to target |
|Qlty     | Target quality is _clear_ or _degraded_  |
|lagQlty  | Last-trial target quality is _clear_ or _degraded_  |
|lagTrgt  | Last-trial target requires _word_ or _nonword_ response |
|rt       | Reaction time [ms] |
...
|P     |Prime   | Prime is _related_ or _unrelated_ to target |
|Q     |Qlty    | Target quality is _clear_ or _degraded_   |
|lQ    |lagQlty | Last-trial target quality is _clear_ or _degraded_  |
|lT    |lagTrgt | Last-trail target requires _word_ or _nonword_ response |
...
contrasts(dat$Prime)     <- contr.sum(2)
...
mm <- model.matrix(~ 1 + Freq*Prime*Qlty*lagQlty*lagTrgt, data=dat)
dat$F  <- mm[,2]
dat$P  <- mm[,3]
dat$Q  <- mm[,4]
dat$lQ <- mm[,5]
...
```{r m_prsm, cache=TRUE}
prsmLMM <- lmer(rrt ~ 1 + F*P*Q*lQ*lT + (1+Q | Subj) + (0+lT | Subj) + (1 | Item) + (0+P | Item), 
