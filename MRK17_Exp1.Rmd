...
|lagTrgt  | Lag target require _word_ or _nonword_ response |
|rt       | Reaction time [ms] |
...
|lT    |lagTrgt | Lag target require _word_ or _nonword_ response |
...
contrasts(dat$lagTrgt) <- contr.sum(2)

# Letter codes for factors, converted to indicator variables
mm <- model.matrix(~ 1 + Freq*TPR*Qlty*lagQlty*lagTrgt, data=dat)
dat$F  <- mm[,2]
dat$T  <- mm[,3]
dat$Q  <- mm[,4]
dat$lQ <- mm[,5]
...
```{r}
dat <- data.frame(dat)
save(dat, file="MRK17_Exp1_xtra.RData")
saveRDS(dat, file="MRK17_Exp1_xtra.Rds" )
write_csv(dat, "MRK17_Exp1_xtra.csv")
```
