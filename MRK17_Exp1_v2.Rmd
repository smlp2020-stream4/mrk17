...
# ... ... check for equality of logLik of cnLMM1 and cnLMM2
anova(cnLMM1, cnLMM2)
```
...
dat$fpqt <- factor(paste(dat$lQ, dat$lT, dat$F, dat$P, sep="."), levels=levels(fpqtC)) # make sure new factor has same order of levels as constructed contrast matrix
contrasts(dat$fpqt) <- contr.hypothesis(fpqtC)


cnLMM4 <- lmer(rrt ~ 1 + fpqt + (1|Subj) + (1|Item), data = dat, REML = FALSE, 
               control=lmerControl(calc.derivs=FALSE))

print(summary(cnLMM4), corr = FALSE)

# ... ... check for equality of logLik of models
anova(cnLMM1, cnLMM2, cnLMM3, cnLMM4 )
```
